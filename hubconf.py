...
from models.backbone import Backbone, Joiner, DINOBackbone # wsod
...
def _make_detr(backbone_name: str, dilation=False, num_classes=91, mask=False, arch=None, patch_size=None):
    hidden_dim = 256
    # wsod
    if backbone_name == "dino":
        backbone = DINOBackbone(arch, patch_size)
    else:
...
# wsod
def detr_dino(pretrained=False, num_classes=91, return_postprocessor=False, arch='vit_small', patch_size=8):
    """
    DETR DINO with 6 encoder and 6 decoder layers.

    Achieves ??/?? AP/AP50 on COCO val5k.
    """
    import os 
    import models.vision_transformer as vits

    model = _make_detr("dino", dilation=False, num_classes=num_classes, arch=arch, patch_size=patch_size)
